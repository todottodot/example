...
public class DogCartQtyUpService {
	...
	public void upCartQty(String kind, HttpServletRequest request){
		...
		/* 장바구니 항목 객체에서 수량을 증가시킬 대상을 kind값으로 비교하여 검색한 후 해당 객체의 수량 증가 */
		for(int i=0; i<cartList.size(); i++) {
			/*
			if(cartList.get(i).getKind().equals(kind)) {
				cartList.get(i).setQty(cartList.get(i).getQty()+1); // 기존 Qty값을 얻어와서 1증가
			}
			*/
			
			Cart cart = cartList.get(i);
			if(cart.getKind().equals(kind)) {
				cart.setQty(cartList.get(i).getQty()+1); // 기존 Qty값을 얻어와서 1증가 후
